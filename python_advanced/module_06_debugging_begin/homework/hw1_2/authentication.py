...
def is_strong_password(password: str) -> bool:
    ...
    for word in words:
        if word.rstrip() in password:
            logger.info(f'incorrect word "{word}" in pasword "{password}"')
            return True
...
if __name__ == "__main__":
    time_format = '%H:%M:%S'
    logging.basicConfig(level=logging.DEBUG, filename='stderr.txt', filemode='w',
    ...
    with open('/usr/share/dict/words', 'r') as file:
        words = list(filter(lambda x: len(x) > 4, file))
