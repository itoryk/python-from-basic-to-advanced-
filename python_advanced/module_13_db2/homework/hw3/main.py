...
def log_bird(
    ...
    cursor.execute(f"INSERT INTO birds (name,count, right_now) VALUES (?,?,?)", data)
...
def check_if_such_bird_already_seen(
    ...
    response = cursor.execute(f"SELECT EXISTS(SELECT * FROM birds WHERE name='{bird_name}')")
...
if __name__ == "__main__":
    ...
    with sqlite3.connect("../homework.db") as connection:
        ...
        cursor.execute("""
                CREATE TABLE IF NOT EXISTS birds (id INTEGER PRIMARY KEY AUTOINCREMENT, 
                name TEXT NOT NULL, count INTEGER NOT NULL, right_now VARCHAR NOT NULL)
            """)
