...
sport_activities = [
    (0, 'футбол'), (1, 'хоккей'), (2, 'шахматы'),
    (3, 'SUP-сёрфинг'), (4, 'бокс'), (5, 'Dota2'),
    (6, 'шахбокс')
]
...
def get_new_schedule(schedule, employees):
    ...
    for day_emp_date in schedule:
        ...
        if sport_activities[day_of_week][1] == employees[id_emp - 1][2]:
            while True:
                new_worker = random.choice(employees)
                if sport_activities[day_of_week][1] != new_worker[2]:
